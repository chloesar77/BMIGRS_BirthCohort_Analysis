...
pheno2=pheno[,c("framid","idtype.x","cohort","sex.x","age65","CAD_event","mean_birth","birthcohort3","birthcohort4","birthcohort2groups","wGRS_941","famid","id","fa","mo","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10","SMK")]
...
m=matrix(pheno2$wGRS.x,nrow=1,ncol=dim(pheno2)[1])
snp=1
snp=as.integer(snp)
chrom=rep(1,each=1)
chrom=as.integer(chrom)
scanID=pheno2$framid
pos=1:1
data=MatrixGenotypeReader(genotype=m,snpID=snp,chromosome=chrom,position=pos,scanID=scanID)
...
#the score test with SPA in Genesis does not allow to test for GxE
#we fit a null model without the BMI GRS and we then test the association of the BMIGRS

nullmod1 <- fitNullModel(iterator, family = "binomial", outcome = "CAD_event", covars=c("sex","SMK","age65","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10"), cov.mat =mat)

assoc1=assocTestSingle(iterator,nullmod1,test="Score.SPA")
OR=exp(assoc1$Est)
ORl=exp(assoc1$Est-1.96*assoc1$Est.SE)
ORu=exp(assoc1$Est+1.96*assoc1$Est.SE)
